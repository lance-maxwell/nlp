...
def load_array(data_arrays, batch_size, is_train=True):
    # 用于构建训练集/测试集的数据加载器，便于批次加载数据和迭代，高效训练模型
    # 参数：
    # data_arrays：包含输入数据张量和标签张量的元组或列表。包含训练数据的输入特征和标签
    # batch_size：每个批次的样本数量
    """Construct a PyTorch data iterator."""
    # 将输入数据转换成pytorch的“TensorDataset”对象(pytorch中的dataset类型)
    dataset = data.TensorDataset(*data_arrays)
    # 通过DataLoader类构建一个数据加载器对象，用于对数据批次加载和迭代。
    # DataLoader会根据batch_size参数将数据集分割成批次，并根据is_train的值决定是否对数据随机打乱。
...
def download_extract(name, folder=None):
    # 下载并解压缩zip或tar文件。name：要下载的文件的URL或者文件名。folder：指定解压缩后的文件夹名称。
    fname = download(name)
    #fname = "E:/data/aclImdb_v1.tar.gz"
    # 调用download函数下载文件，返回下载的文件名fname。
    # 再根据文件名的扩展名判断文件类型，并使用相应的解压缩方法进行解压缩。
    ...
    fp.extractall(base_dir) # 解压到指定目标文件夹中
